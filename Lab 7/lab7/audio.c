			...
			*(HEX_ptr) = 0x7938776e;   /*display 'PLAY'*/
			...
			{	
				while ( (fifospace & 0x00FF0000) && (buffer_index < BUF_SIZE) )
				{
					...
					if ( buffer_index > 7000 ) {
						*(audio_ptr + 2) = left_buffer[buffer_index-7000] + (left_buffer[buffer_index]/2);
						*(audio_ptr + 3) = right_buffer[buffer_index-7000] + (right_buffer[buffer_index]/2);
					}else {
