...
int main()
{
	...
	*(HEX_ptr) = 0x00000000;   /*display '0000'*/
	...
	while(1)
	{
			...
			*(HEX_ptr) = 0x00507939;   /*display 'REC'*/
			...
			if ( (fifospace & 0x000000FF) > BUF_THRESHOLD )  //see if there is room to record
			{
				while ( (fifospace & 0x000000FF) && (buffer_index < BUF_SIZE) )
				{
					...
					buffer_index = buffer_index + 1; //increment buffer index (make 2 for slo-mo)
		...
		{
			*(HEX_ptr) = 0x3338776e;   /*display 'PLAY'*/
			...
			{	
				while ( (fifospace & 0x00FF0000) && (buffer_index < BUF_SIZE) )
				{
					...
					*(audio_ptr + 3) = right_buffer[buffer_index];
					buffer_index = buffer_index + 1; //increment buffer index (make 2 for chipmunk)
